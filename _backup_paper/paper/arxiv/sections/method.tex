...
\caption{Architecture of the Thermodynamic Persona Engine (TPE). \textbf{Forward pass} (top row): Time Metabolism applies frustration decay and hunger accumulation between turns; the Input Critic extracts interpersonal dimensions $(a, d, e)$ from user input, which update the Drive Metabolism (5D frustration vector $\mathbf{f}$, total $F = \sum f_d$). Signal Generation maps $\mathbf{f}$ to 8D behavioral signals $\mathbf{s} \in [0,1]^8$ via a genome network. The total frustration $F$ sets the temperature ($\sigma = 0.05 \cdot F$) for Gaussian noise injection into $\mathbf{s}$. Gravitational Retrieval selects top-$k$ memory examples weighted by effective mass $m_\text{eff}$, which decays via Hawking radiation. The Actor LLM generates responses conditioned on the noise-perturbed signals and retrieved memories. \textbf{Feedback loop} (bottom row): The Reply Critic scores each response to produce reward $r$; the Crystallization Gate writes successful interactions ($r > 0.3$) into the Memory Pool; Hebbian learning updates the genome network weights.}
