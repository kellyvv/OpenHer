...
"""
╔══════════════════════════════════════════════════════════════════════╗
║  🧬 Inner Monologue V2.1 × qwen3-max — 模型对比 🧬                  ║
║                                                                      ║
║  同样的 信号冲突拦截 + 中文方括号格式                                 ║
║  唯一变量：LLM 从 M2-her → qwen3-max                                ║
╚══════════════════════════════════════════════════════════════════════╝
"""
...
def call_qwen(system_prompt: str, user_msg: str) -> str:
    payload = json.dumps({
        "model": MODEL,
        "messages": [
    ...
    try:
        with urllib.request.urlopen(req, context=ssl.create_default_context(), timeout=30) as resp:
            ...
            if data.get("choices") and data["choices"][0].get("message"):
                ...
                # qwen3-max 可能用 <think>...</think> 包裹思考过程
...
def run():
    print(f"""
{C.BOLD}══════════════════════════════════════════════════════════════
  Inner Monologue V2.1 × qwen3-max
  同样的信号冲突拦截，唯一变量: LLM
=============================================================={C.RESET}
""")
    ...
    print(f'{C.BOLD}{"─" * 60}{C.RESET}')
    print(f'  测试完成！对比 M2-her 版的结果看差异。')
    print(f'{C.BOLD}{"─" * 60}{C.RESET}')
