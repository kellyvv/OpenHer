"""
OpenAI gpt-5.4-mini-2026-03-17 Test — 3-Layer Persona Engine Verification.
...
async def layer1_persona(persona_id, loader):
    ...
    agent = ChatAgent(
        persona=persona, llm=llm, user_id="test_gpt54mini_l1",
    ...
    for i, msg in enumerate(L1_MSGS, 1):
        ...
        turn_data.append({
            "turn": i, "msg": msg, "reply": reply, "monologue": mono,
            "modality": modality, "reward": r, "temp": t,
    ...
    return {
        ...
        "personal": sm_final['personal_count'], "turn_data": turn_data,
    }
...
async def layer2_metabolism(persona_id, loader):
    ...
    agent = ChatAgent(
        persona=persona, llm=llm, user_id="test_gpt54mini_l2",
        user_name="测试者", genome_seed=seed, genome_data_dir=GENOME_DATA_DIR,
    )
    agent.pre_warm()
    ...
    for i, msg in enumerate(L2_MSGS, 1):
        ...
        turn_data.append({
            ...
            "reward": r, "temp": t, "frust": f,
            "frustration_detail": {d: round(v, 4) for d, v in agent.metabolism.frustration.items()},
        })
    ...
    return {
        "persona": persona_id, "name": persona.name, "mbti": persona.mbti,
        "max_reward": max_r, "rewards": rewards, "temps": temps, "frusts": frusts,
        "passed": passed, "turn_data": turn_data,
    }
...
async def layer3_hebbian(persona_id, loader):
    ...
    llm = make_llm()
    seed = hash(persona_id) % 100000
    user_id = f"test_gpt54mini_persist_{persona_id}"
    ...
    print(f"  [S1] genesis={genesis_count}  personal={seeds_before}")
    print(f"  [S1] NN age={agent1.agent.age}  interactions={agent1.agent.interaction_count}")
    ...
    for i, msg in enumerate(L3_DEEP_MSGS, 1):
        ...
        s1_rewards.append(r)
        reply = result.get("reply", "")
        print(f"  [S1 T{i}] {msg[:20]}… → reward={r:+.4f} temp={t:.4f}  💬 {reply[:80]}...")
        s1_turn_data.append({
            "turn": i, "msg": msg, "reply": reply, "reward": r, "temp": t,
        })
    ...
    print(f"  [S1] NN age={agent1.agent.age}  total_reward={agent1.agent.total_reward:.3f}")
    print(f"  [S1] metabolism total={agent1.metabolism.total():.4f}")
    ...
    print(f"\n  [S2] restored: personal={s2_stats['personal_count']} age={agent2.agent.age} metab={s2_frust:.4f}")
    ...
    for i, msg in enumerate(L3_FOLLOWUP, 1):
        ...
        s2_rewards.append(r)
        reply = result.get("reply", "")
        print(f"  [S2 T{i}] {msg[:20]}… → reward={r:+.4f}  💬 {reply[:80]}...")
        s2_turn_data.append({
            "turn": i, "msg": msg, "reply": reply, "reward": r,
        })
    ...
    if s2_frust < 0.01:
        errors.append(f"S2 metabolism total={s2_frust:.4f} < 0.01")

    s2_age = agent2.agent.age
    s1_age = agent1.agent.age
    if s2_age < s1_age:
        errors.append(f"S2 age={s2_age} < S1 age={s1_age}")

    passed = len(errors) == 0
    print(f"  {'✅ PASS' if passed else '❌ FAIL: ' + str(errors)}")
    return {
        "persona": persona_id, "name": persona.name, "mbti": persona.mbti,
        "s1_crystal": crystal, "s1_rewards": s1_rewards,
        "s2_persist": s2_final['personal_count'], "s2_rewards": s2_rewards,
        "s2_metabolism": s2_frust, "s1_age": s1_age, "s2_age": s2_age,
        "passed": passed, "s1_turn_data": s1_turn_data, "s2_turn_data": s2_turn_data,
    }
...
async def main():
    ...
    # ── Layer 3 ──
    print(f"\n\n{'▓'*60}")
    print("  LAYER 3: HEBBIAN + CROSS-SESSION (6h, 3×(5+2))")
    print(f"{'▓'*60}")
    l3 = []
    for pid in TEST_PERSONAS:
        try:
    ...
    # ── Save raw data as JSON for analysis ──
    output = {
        "model": MODEL, "provider": PROVIDER,
        "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
        "layer1": l1, "layer2": l2, "layer3": l3,
    }
    output_path = os.path.join(BASE_DIR, "docs", "benchmark", "gpt54mini_3layer_raw.json")
